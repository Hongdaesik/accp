    ...
    if ( Array.isArray( block ) ) {
      ...
      switch ( d ) {
        ...
        case 3: {
          ...
          if ( !Array.isArray( block ) || new RegExp( /^\s*([0-9]+)\s{1}(.*?)$/g ).test( block.input ) == false ) return setLog(
      ...
      if ( Array.isArray( block ) ) {
        ...
        switch ( d ) {
          ...
          case 2: {
            ...
            if ( !Array.isArray( block ) ) return setLog(
      ...
      switch ( d ) {
        ...
        case 3: {
          ...
          if ( !Array.isArray( variable ) ) return setLog(
            ...
            'Struct data must have the structure "([0-9a-zA-Z가-힣-_!=><]+) (.*?)".', 'brightRed', [

              [ p, l ]
            ]
          )

          if ( new RegExp( /^[0-9a-zA-Z가-힣-_!=><]+$/g ).test( variable[ 1 ] ) == false ) return setLog(

            'Struct data must have the structure "([0-9a-zA-Z가-힣-_!=><]+) (.*?)".', 'brightRed', [

              [ p, l ], variable.input.replace( /([^\s]+)\s{1}(.*?)$/i, ( _, m1, m2 ) => `${ m1.replace( /[^0-9a-zA-Z가-힣-_\s!=><]/g, '' ) } ${ m2 }` )
            ]
          )
        ...
        switch ( d ) {
          ...
          case 3: {
            ...
            if ( RegExp( /^(COMP|GET|PUT|POST|PATCH|DELETE)\s{1}[^{](.*?)$/g ).test( variable.input ) == false ) return setLog(
              ...
              'Only the following instructions are allowed for reserved words, and bracket characters are not allowed.', 'brightRed', [
            ...
            switch ( b?.NAME ) {
              ...
              case 'RES': {
                ...
                if ( d > 4 ) {
                  ...
                  if ( !Array.isArray( variable ) || new RegExp( /^\s*[0-9a-zA-Z가-힣-_!=><]+$/g ).test( variable[ 1 ] ) == false ) return setLog(

                    `${ b.NAME } must have the structure "([0-9a-zA-Z가-힣-_!=><]+) (.*?)".`, 'brightRed', [
